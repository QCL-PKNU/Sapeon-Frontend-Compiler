...
std::pair<int64_t, int64_t> GetMinAndMaxContiguousAxes(
    int64_t rank, const std::vector<int64_t>& dims,
  ...
  const std::vector<int64_t> axes = [&original_axes, rank]() {
    ...
    std::for_each(result.begin(), result.end(), [rank](int64_t& axis) {
  ...
  for (auto a = axes.begin(), b = axes.begin() + 1; b != axes.end(); ++a, ++b) {
    assert(*a != *b);
...
ApplicableMatrixReduction get_applicable_matrix_reduction(
    ...
    gsl::span<const int64_t> original_axes, int& m_out, int& n_out) {
  if (cudnn_reduce_op != CUDNN_REDUCE_TENSOR_ADD &&
      cudnn_reduce_op != CUDNN_REDUCE_TENSOR_AVG) {
    return ApplicableMatrixReduction::None;
  }

  // Remove all dims with value 1. This can help to optimize case like:
  // dims=[2,3,1,4,1,5] and axes=[0,2,4], which is same as dims=[2,3,4,5] and
  // axes=[0].
  ...
  for (size_t i = 0; i < dims.size(); i++) {
    if (dims[i] != 1) {
      ...
      if (original_axes_set.find(gsl::narrow<int64_t>(i)) !=
          original_axes_set.end()) {
  ...
  const auto min_and_max_axes =
      GetMinAndMaxContiguousAxes(rank, new_dims, new_axes);
  if (!min_and_max_axes.first == std::numeric_limits<int64_t>::lowest()) {
    return ApplicableMatrixReduction::None;
  }
  ...
  return axes_from_beginning ? ApplicableMatrixReduction::Rows

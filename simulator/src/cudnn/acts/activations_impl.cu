...
struct OP_Relu : public CtxRelu {
  __device__ __inline__ T operator()(const T& a) const { return _Max(a, (T)0); }
};
...
struct OP_Sigmoid : public CtxSigmoid {
  __device__ __inline__ T operator()(const T& a) const {
    return a > T(0) ? (T)1 / ((T)1. + _Exp(-_Abs(a)))
                    : (T)1 - (T)1 / ((T)1 + _Exp(-_Abs(a)));
  }
};
...
struct OP_Tanh : public CtxTanh {
  __device__ __inline__ T operator()(const T& a) const { return _Tanh(a); }
};

template <typename T>
struct OP_Mish : public CtxMish {
  __device__ __inline__ T operator()(const T& a) const { return _Mish(a); }
};
...
struct OP_Celu : public CtxCelu {
  __device__ __inline__ T operator()(const T& a) const {
    return _Max((T)0, (T)a) +
           _Min((T)0, (T)alpha * (_Exp((T)a / (T)alpha) - (T)1));
  }
};

#define UNARY_ACTIVATION_IMPL(name)                                        \
  UNARY_ACTIVATION_IMPL_DECLARATION(name) {                                \
    UnaryElementWiseImpl(stream, input_data, output_data,                  \
                         *reinterpret_cast<const OP_##name<T>*>(func_ctx), \
                         count);                                           \
  }

#define SPECIALIZED_UNARY_ACTIVATION_IMPL(name, T)                        \
  template void Impl_##name<T>(cudaStream_t stream, const T* input_data,  \
                               T* output_data, const Ctx##name* func_ctx, \
                               size_t count);

...
__global__ void _UpampleNearestKernel(
    const TArray<int64_t> input_pitches,
    const TArray<fast_divmod> output_div_pitches,
    const TArray<fast_divmod> scales_div, const T* __restrict__ input_data,
    T* __restrict__ output_data, const size_t N) {
...
// This is the common use-case where the 4-D input (batched multi-channel
// images) is usually of shape [N, C, H, W] and the scales are [1.0, 1.0,
// height_scale, width_scale]
template <typename T>
__global__ void _UpampleBilinear4DInputKernel(
    const int64_t input_dim2, const TArray<int64_t> input_pitches,
    const TArray<fast_divmod> output_div_pitches,
    const TArray<fast_divmod> scales_div, const T* __restrict__ input_data,
    T* __restrict__ output_data, const size_t N) {
  CALCULATE_ELEMENTWISE_INDEX_OR_EXIT(id, N);
  ...
  input_index = index_of_dim0 * input_pitches[0] +
                ...
                index_of_input_dim2 * input_pitches[2] + index_of_input_dim3;
...
template <typename T>
__global__ void _UpampleBilinear2DInputKernel(
    const int64_t input_dim0, const TArray<int64_t> input_pitches,
    const TArray<fast_divmod> output_div_pitches,
    const TArray<fast_divmod> scales_div, const T* __restrict__ input_data,
    T* __restrict__ output_data, const size_t N) {
  CALCULATE_ELEMENTWISE_INDEX_OR_EXIT(id, N);
...
void UpampleImpl(cudaStream_t stream, const UpsampleMode upsample_mode,
  ...
  int blocksPerGrid =
      (int)(ceil(static_cast<float>(N) / GridDim::maxThreadsPerBlock));
  if (UpsampleMode::NN == upsample_mode) {
    if (rank == 4) {
      _UpampleNearestKernel<T, 4>
          <<<blocksPerGrid, GridDim::maxThreadsPerBlock, 0, stream>>>(
              input_pitches, output_div_pitches, scales_div, input_data,
              output_data, N);
    } else if (rank == 3) {
      _UpampleNearestKernel<T, 3>
          <<<blocksPerGrid, GridDim::maxThreadsPerBlock, 0, stream>>>(
              input_pitches, output_div_pitches, scales_div, input_data,
              output_data, N);
    } else if (rank == 2) {
      _UpampleNearestKernel<T, 2>
          <<<blocksPerGrid, GridDim::maxThreadsPerBlock, 0, stream>>>(
              input_pitches, output_div_pitches, scales_div, input_data,
              output_data, N);
    } else if (rank == 1) {
      _UpampleNearestKernel<T, 1>
          <<<blocksPerGrid, GridDim::maxThreadsPerBlock, 0, stream>>>(
              input_pitches, output_div_pitches, scales_div, input_data,
              output_data, N);
    } else {
      LOG(ERROR) << "Unsupported rank by the Upsample CUDA kernel. Input rank: "
                 << rank;
      assert(false);
    }
  } else if (UpsampleMode::LINEAR == upsample_mode) {
    if (rank == 4) {
      _UpampleBilinear4DInputKernel<T>
          <<<blocksPerGrid, GridDim::maxThreadsPerBlock, 0, stream>>>(
              input_dim2, input_pitches, output_div_pitches, scales_div,
              input_data, output_data, N);
    } else if (rank == 2) {
      _UpampleBilinear2DInputKernel<T>
          <<<blocksPerGrid, GridDim::maxThreadsPerBlock, 0, stream>>>(
              input_dim2, input_pitches, output_div_pitches, scales_div,
              input_data, output_data, N);
    } else {
    ...
    // Should never encounter this as Upsample only supports 'Nearest' and
    // 'Linear' modes. But if we do encounter this it is best to throw instead
    // of returning silently.
    LOG(ERROR) << "Unsupported mode: " << upsample_mode;
    assert(false);
  }
}

#define SPECIALIZED_IMPL(T)                                       \
  template void UpampleImpl<T>(                                   \
      cudaStream_t stream, const UpsampleMode upsample_mode,      \
      const size_t rank, const int64_t input_dim2,                \
      const TArray<int64_t>& input_pitches,                       \
      const TArray<fast_divmod>& output_div_pitches,              \
      const TArray<fast_divmod>& scales_div, const T* input_data, \

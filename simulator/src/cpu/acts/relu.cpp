...
void SCOPE::ActivationForward(Layer &layer) {
  ...
  auto activation = [](auto x) -> decltype(x) { return x > 0 ? x : 0; };
...
void SCOPE::ActivationQuantForward(Layer &layer) {
  ...
  if (itype == dty::DataType::INT16 && otype == dty::DataType::INT16) {
    Activation::ActivationForward<int16_t, int16_t>(std::move(activation));
  } else if (itype == dty::DataType::INT8 && otype == dty::DataType::UINT8) {
    Activation::ActivationForward<int8_t, uint8_t>(std::move(activation));
  } else if (itype == dty::DataType::UINT8 && otype == dty::DataType::UINT8) {
    Activation::ActivationForward<uint8_t, uint8_t>(std::move(activation));
  } else {

...
#ifndef CUDNN_COMMON_ORT_MUTEX_HPP
#define CUDNN_COMMON_ORT_MUTEX_HPP
...
// A: OrtMutex supports static initialization but std::mutex doesn't. Static
// initialization helps us prevent the "static initialization order problem".

// Q: Why std::mutex can't make it?
// A: VC runtime has to support Windows XP at ABI level. But we don't have such
// requirement.
...
class OrtCondVar {
 ...
 public:
  ...
  void wait(std::unique_lock<OrtMutex>& lk) {
    if (SleepConditionVariableSRW(&native_cv_object,
                                  lk.mutex()->native_handle(), INFINITE,
                                  0) != TRUE) {
      std::terminate();
    }
  }
  ...
  /**
   * returns cv_status::timeout if the wait terminates when Rel_time has
   * elapsed. Otherwise, the method returns cv_status::no_timeout.
   * @param cond_mutex A unique_lock<OrtMutex> object.
   * @param rel_time A chrono::duration object that specifies the amount of time
   * before the thread wakes up.
   * @return returns cv_status::timeout if the wait terminates when Rel_time has
   * elapsed. Otherwise, the method returns cv_status::no_timeout
   */
  template <class Rep, class Period>
  std::cv_status wait_for(std::unique_lock<OrtMutex>& cond_mutex,
...
template <class Rep, class Period>
std::cv_status OrtCondVar::wait_for(
    std::unique_lock<OrtMutex>& cond_mutex,
    const std::chrono::duration<Rep, Period>& rel_time) {
  // TODO: is it possible to use nsync_from_time_point_ ?
  using namespace std::chrono;
  if (rel_time <= duration<Rep, Period>::zero()) return std::cv_status::timeout;
  using SystemTimePointFloat =
      time_point<system_clock, duration<long double, std::nano> >;
...
#endif  // CUDNN_COMMON_ORT_MUTEX_HPP

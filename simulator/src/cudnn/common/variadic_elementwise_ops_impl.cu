...
#include "cudnn/common/variadic_elementwise_ops_impl.cuh"
...
#define DEFINE_TRAITS(VariadicElementwiseOpTag, ImplName)                      \
  ...
  struct VariadicElementwiseOpTraits<T, VariadicElementwiseOpTag> {            \
    using ScalarComputeFunctor = OP_##ImplName<T, T, T>;                       \
                                                                               \
    static void ComputeFn(cudaStream_t stream,                                 \
      ...
      Impl_##ImplName(stream, output_rank_or_simple_broadcast,                 \
                      lhs_padded_strides, lhs_data, rhs_padded_strides,        \
                      rhs_data, fdm_output_strides, fdm_H, fdm_C, output_data, \
                      count);                                                  \
    }                                                                          \
  };
...
void Impl_NoBroadcastInputBatch(cudaStream_t stream,
  ...
  VariadicElementWiseNoBroadcastInputBatchImpl<
      T,
      typename VariadicElementwiseOpTraits<
          T, VariadicElementwiseOpTag>::ScalarComputeFunctor,
      k_max_input_batch_size>(
      stream,
      typename VariadicElementwiseOpTraits<
          T, VariadicElementwiseOpTag>::ScalarComputeFunctor{},
      count, input_data_batch, output_data);
}
...
#define SPECIALIZE_IMPL(T, VariadicElementwiseOpTag)                           \
  ...
  template void Impl_NoBroadcastInputBatch<T, VariadicElementwiseOpTag>(       \
      cudaStream_t stream, InputBatchArray<T> input_data_batch,                \
      T * output_data, size_t count);

...
#ifndef CUDNN_COMMON_UNARY_ELEMENTWISE_IMPL_CUH
#define CUDNN_COMMON_UNARY_ELEMENTWISE_IMPL_CUH
...
template <typename InT, typename OutT, typename FuncT, int NumThreadsPerBlock,
          int NumElementsPerThread>
__global__ void _UnaryElementWise(const InT* input_data, OutT* output_data,
...
void UnaryElementWiseImpl(cudaStream_t stream, const InT* input_data,
                          OutT* output_data, const FuncT& func, size_t count) {
  ...
  _UnaryElementWise<InT, OutT, FuncT, GridDim::maxThreadsPerBlock,
                    GridDim::maxElementsPerThread>
      <<<blocksPerGrid, GridDim::maxThreadsPerBlock, 0, stream>>>(
          input_data, output_data, func, N);
}

}  // namespace Cudnn

#endif  // CUDNN_COMMON_UNARY_ELEMENTWISE_IMPL_CUH

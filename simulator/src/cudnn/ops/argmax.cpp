...
std::shared_ptr<Tensor> SCOPE::Forward(cudnnHandle_t &handle, Layer &layer) {
  ...
  ReduceComputeCore<Type, CUDNN_REDUCE_TENSOR_FLATTENED_INDICES>(
      ...
      output_shape, (Type *)data_output_, CUDNN_REDUCE_TENSOR_MAX, axes, false,
...
#define DEFINE_ARG_MAX_INT(TYPE, CUDNN_DATA_TYPE)                              \
  template <>                                                                  \
  std::shared_ptr<Tensor> Cudnn::ArgMax<TYPE, CUDNN_DATA_TYPE>::Forward(       \
    ...
    Impl_Cast<CudaT, float>(stream,                                            \
                            ...
                            temp_X, x_shape.Size());                           \
                                                                               \
    assert(reduce_desc.Set(CUDNN_REDUCE_TENSOR_MAX, cudnn_type_X,              \

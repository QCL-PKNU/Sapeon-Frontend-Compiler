    ...
    if (blockDim.y > 1) {
      ...
      for (int offset = blockDim.y / 2; offset > 0; offset /= 2) {
        ...
        if (threadIdx.x == 0 && threadIdx.y < offset) {
          ...
          cuChanOnlineSum<U, simplified>(muB, sigma2B, countB, mu, sigma2,
                                         count);
        }
        __syncthreads();
      }
...
__device__ void cuWelfordMuSigma2(const half* __restrict__ vals, const int n1,
                                  const int n2, const int i1, float& mu,
                                  float& sigma2, float* buf) {
  // Assumptions:
  // 1) blockDim.x == GPU_WARP_SIZE
  // 2) Tensor is contiguous
  // 3) 2*blockDim.y*sizeof(U)+blockDim.y*sizeof(int) shared memory available.
...
#pragma unroll
    for (int stride = GPU_WARP_SIZE / 2; stride > 0; stride /= 2) {
      ...
      cuChanOnlineSum<float, simplified>(muB, sigma2B, countB, mu, sigma2,
                                         count);
    }
  ...
  for (int i1 = blockIdx.y; i1 < n1; i1 += gridDim.y) {
    ...
    for (int i = thrx; i < n2; i += numx) {
      ...
      if (simplified) {
        ...
        ovals[i] =
            gamma_i * static_cast<V>(c_inv_std_dev * (curr - mu)) + beta_i;
      }
    }
    if (threadIdx.x == 0 && threadIdx.y == 0) {
...
void HostApplyLayerNorm(const cudaDeviceProp& prop, cudaStream_t stream,
  ...
  cuApplyLayerNorm<T, U, V, simplified><<<blocks, threads, nshared, stream>>>(
      output, mean, inv_std_dev, input, n1, n2, U(epsilon), gamma, beta);
}

#define LAYERNORM_LINEAR_IMPL(T, U, V, simplified)                         \
  template void HostApplyLayerNorm<T, U, V, simplified>(                   \
      const cudaDeviceProp& prop, cudaStream_t stream, V* output, U* mean, \
      U* inv_std_dev, const T* input, int n1, int n2, double epsilon,      \
      const V* gamma, const V* beta);

...
template int64_t accumulate<const int64_t*, int64_t>(const int64_t* first,
                                                     const int64_t* last,
                                                     int64_t init);

struct NearestPixel_SIMPLE {
  __device__ __forceinline__ int operator()(float x_original,
                                            bool is_down_sampling) const {
    if (is_down_sampling) {
...
struct TransformCoordinate_ASYMMETRIC {
  __device__ __forceinline__ float operator()(float x_resized, float x_scale,
                                              float, float, float,
                                              float) const {
    ...
    auto orig =
        length_resized > 1
            ? roi_start * (length_original - 1) +
                  (x_resized * (roi_end - roi_start) * (length_original - 1)) /
                      (length_resized - 1)
...
#define DISPATCH_RESIZE_COORDINATE_TRANSFORMATION_MODE(TYPE, ...)             \
  [&] {                                                                       \
    ...
    switch (the_type) {                                                       \
      CASE_TYPE_COORD(ResizeCoordinateTransformationMode::HALF_PIXEL,         \
                      TransformCoordinate_HALF_PIXEL, __VA_ARGS__)            \
      CASE_TYPE_COORD(ResizeCoordinateTransformationMode::ASYMMETRIC,         \
      ...
      CASE_TYPE_COORD(                                                        \
          ResizeCoordinateTransformationMode::TF_HALF_PIXEL_FOR_NN,           \
      ...
      default:                                                                \
        LOG(ERROR) << "unknown ResizeCoordinateTransformationMode";           \
        assert(false);                                                        \
    }                                                                         \
  }()
...
#define DISPATCH_RESIZE_NEAREST_MODE(TYPE, ...)                             \
  [&] {                                                                     \
    ...
    /* don't use TYPE again in case it is an expensive or side-effect op */ \
    switch (the_type) {                                                     \
      CASE_TYPE_NEAREST(ResizeNearestMode::SIMPLE, NearestPixel_SIMPLE,     \
                        __VA_ARGS__)                                        \
      CASE_TYPE_NEAREST(ResizeNearestMode::ROUND_PREFER_FLOOR,              \
                        NearestPixel_ROUND_PREFER_FLOOR, __VA_ARGS__)       \
      CASE_TYPE_NEAREST(ResizeNearestMode::ROUND_PREFER_CEIL,               \
                        NearestPixel_ROUND_PREFER_CEIL, __VA_ARGS__)        \
      CASE_TYPE_NEAREST(ResizeNearestMode::FLOOR, NearestPixel_FLOOR,       \
                        __VA_ARGS__)                                        \
      CASE_TYPE_NEAREST(ResizeNearestMode::CEIL, NearestPixel_CEIL,         \
                        __VA_ARGS__)                                        \
      default:                                                              \
        LOG(ERROR) << "unknown ResizeNearestMode";                          \
        assert(false);                                                      \
    }                                                                       \
  }()
...
template <typename T, typename CudaFunctionOriginalCoordinate,
          typename CudaFunctionNearestPixel>
__global__ void _ResizeNearestMappingKernel2D(
    const int input_height, const int input_width, const int output_height,
    const int output_width, const float scales_height, const float scales_width,
  ...
  if (id >= 0 && id < output_height) {  // for Height
    ...
    if (scales_height == 1.0f) {
      ...
      float orig_coord = transform_coordinate(
          static_cast<float>(dim), scales_height,
          static_cast<float>(output_height), static_cast<float>(input_height),
          roi_start_height, roi_end_height);
      dims_mapping[id].extrapolate_ =
          static_cast<int>(extrapolation_enabled &&
                           (orig_coord < 0.f ||
                            orig_coord > static_cast<float>(input_height - 1)));
    ...
    if (scales_width == 1.0f) {
      ...
      float orig_coord = transform_coordinate(
          static_cast<float>(dim), scales_width,
          static_cast<float>(output_width), static_cast<float>(input_width),
          roi_start_width, roi_end_width);
      dims_mapping[id].extrapolate_ =
          static_cast<int>(extrapolation_enabled &&
                           (orig_coord < 0.f ||
                            orig_coord > static_cast<float>(input_width - 1)));
...
__global__ void _ResizeNearestMappingKernel(
    const size_t rank, const TArray<int64_t> input_shape,
    const TArray<int64_t> output_shape, const TArray<float> scales,
    const TArray<float, 10> roi, const size_t total_dim_sum,
    bool extrapolation_enabled,
    const CudaFunctionOriginalCoordinate& transform_coordinate,
    const CudaFunctionNearestPixel& calc_nearest_pixel, int64_t* prefix_dim_sum,
    NearestMappingInfo* dims_mapping) {
  ...
  for (int axis = 0; axis < rank; ++axis) {
    ...
    if (id >= dim_sum && id < dim_sum + output_shape[axis]) {
      ...
      if (scales[axis] == 1.0f) {
        ...
        float orig_coord = transform_coordinate(
            static_cast<float>(dim), scales[axis],
...
__global__ void _ResizeNearestKernel2D(
  ...
  if (UseExtrapolation) {
    if (dims_mapping[h].extrapolate_ +
        dims_mapping[output_height + w].extrapolate_) {
...
__global__ void _ResizeNearestKernel(
    const int rank, const TArray<int64_t> input_strides,
    const TArray<fast_divmod> output_div_pitches, const T* input_data,
    T* output_data, const size_t N, const T extrapolation_value,
    const int64_t* prefix_dim_sum, const NearestMappingInfo* dims_mapping) {
  CALCULATE_ELEMENTWISE_INDEX_OR_EXIT(id, N);
  ...
  output_data[id] =
      extrapolation_occured ? extrapolation_value : input_data[input_index];
}
...
__global__ void _ResizeBilinearCoordinateMapping(
    int64_t input_height, int64_t input_width, int64_t output_height,
    int64_t output_width, float scale_height, float scale_width,
    float roi_height_start, float roi_height_end, float roi_width_start,
    float roi_width_end, const size_t SumHW, bool extrapolation_enabled,
    const CudaFunctionOriginalCoordinate& transform_coordinate,
  ...
  if (id < output_height) {  //  y = id
    ...
    dims_mapping[id].extrapolate_ =
        (int)(extrapolation_enabled &&
              (input_y < 0 || input_y > static_cast<float>(input_height - 1)));
    ...
    dims_mapping[id].weight_ =
        (y_int >= input_height - 1) ? 0.5f : input_y - y_int;
  } else {  // x = id - output_height
    float input_x = scale_width == 1
                        ? static_cast<float>(id - output_height)
    ...
    dims_mapping[id].extrapolate_ =
        (int)(extrapolation_enabled &&
              (input_x < 0 || input_x > static_cast<float>(input_width - 1)));
    ...
    dims_mapping[id].weight_ =
        (x_int >= input_width - 1) ? 0.5f : input_x - x_int;
  }
}

// The following method supports a 2-D or 4-D input in 'Linear mode'. Last two
// dimension is [H, W]. the scale values for the outer dimensions except last
// two are 1.
template <typename T>
__global__ void _ResizeBilinearKernel(
    int64_t input_height, int64_t input_width, int64_t output_height,
    int64_t output_width, fast_divmod div_output_width,
    fast_divmod div_output_image, const T* input_data, T* output_data,
    const size_t N, const T extrapolation_value,
    LinearMappingInfo* dims_mapping) {
  CALCULATE_ELEMENTWISE_INDEX_OR_EXIT(id, N);
  ...
  if (dims_mapping[output_y].extrapolate_ ||
      dims_mapping[output_x + output_height].extrapolate_) {
    output_data[id] = extrapolation_value;
    return;
  }
  ...
  T x11 = end_of_w
              ? x01
              : (end_of_h ? x10 : input_data[input_index + input_width + 1]);

  float y_offset_1 = 1.0f - y_offset_0;
  float x_offset_1 = 1.0f - x_offset_0;
  output_data[id] = x00 * static_cast<T>(y_offset_1 * x_offset_1) +
...
__global__ void _ResizeTrilinearCoordinateMapping(
    ...
    float roi_depth_start, float roi_depth_end, float roi_height_start,
    float roi_height_end, float roi_width_start, float roi_width_end,
  ...
  if (id < output_depth) {  //  z = id
    ...
    dims_mapping[id].weight_ =
        (z_int >= input_depth - 1) ? 0.5f : input_z - z_int;
  } else if (id >= output_depth &&
             id < (output_depth + output_height)) {  //  y = id - output_depth
    float input_y = scale_height == 1
                        ? static_cast<float>(id - output_depth)
                        : transform_coordinate(
                              static_cast<float>(id - output_depth),
    ...
    float input_x =
        scale_width == 1
            ? static_cast<float>(id - output_depth - output_height)
            : transform_coordinate(
                  static_cast<float>(id - output_depth - output_height),
                  scale_width, static_cast<float>(output_width),
                  static_cast<float>(input_width), roi_width_start,
...
// The following method supports a 3-D or 5-D input in 'Linear mode'. Last two
// dimension is [D, sH, W]. the scale values for the outer dimensions except
// last two are 1.
template <typename T>
__global__ void _ResizeTrilinearKernel(
    int64_t input_depth, int64_t input_height, int64_t input_width,
    int64_t output_depth, int64_t output_height, int64_t output_width,
    fast_divmod div_output_height, fast_divmod div_output_width,
    fast_divmod div_output_image, const T* input_data, T* output_data,
    const size_t N, const T extrapolation_value,
    LinearMappingInfo* dims_mapping) {
  CALCULATE_ELEMENTWISE_INDEX_OR_EXIT(id, N);
  ...
  float x_offset_0 =
      dims_mapping[output_x + output_depth + output_height].weight_;
  int x_int = dims_mapping[output_x + output_depth + output_height].origin_;

  input_index +=
      z_int * input_height * input_width + y_int * input_width + x_int;
  ...
  T x110 = end_of_w
               ? x010
               : (end_of_h ? x100 : input_data[input_index + input_width + 1]);
  ...
  T x111 = end_of_w
               ? x011
               : (end_of_h ? x101 : input_data[input_index + input_width + 1]);
...
__device__ __forceinline__ float CubicInterpolationRowwise(
  ...
  return coeff0 * static_cast<float>(
                      image[row_index + max(0, min(x - 1, input_width - 1))]) +
         coeff1 * static_cast<float>(
                      image[row_index + max(0, min(x, input_width - 1))]) +
         coeff2 * static_cast<float>(
                      image[row_index + max(0, min(x + 1, input_width - 1))]) +
         coeff3 * static_cast<float>(
                      image[row_index + max(0, min(x + 2, input_width - 1))]);
}
...
__global__ void _ResizeCubicCoordinateMapping(
  ...
  int max_input_coord =
      static_cast<int>(is_y_axis ? input_height : input_width);

  float scale = is_y_axis ? scale_height : scale_width;
  float input_coordinat =
      scale == 1
          ? (is_y_axis ? id : id - output_height)
          : transform_coordinate(
                static_cast<float>(is_y_axis ? id : id - output_height), scale,
  ...
  float coeff_0 = static_cast<float>(
      ((cubic_coeff_a * (s_coord + 1) - 5 * cubic_coeff_a) * (s_coord + 1) +
       8 * cubic_coeff_a) *
          (s_coord + 1) -
      4 * cubic_coeff_a);
  float coeff_1 =
      static_cast<float>(((cubic_coeff_a + 2) * s_coord - (cubic_coeff_a + 3)) *
                             s_coord * s_coord +
                         1);
  float coeff_2 = static_cast<float>(
      ((cubic_coeff_a + 2) * (1 - s_coord) - (cubic_coeff_a + 3)) *
          (1 - s_coord) * (1 - s_coord) +
      1);
  float coeff_3 = static_cast<float>(
      ((cubic_coeff_a * (2 - s_coord) - 5 * cubic_coeff_a) * (2 - s_coord) +
       8 * cubic_coeff_a) *
          (2 - s_coord) -
      4 * cubic_coeff_a);
  if (exclude_outside) {
    coeff_0 =
        (coord_int - 1 < 0 || coord_int - 1 >= max_input_coord) ? 0.0 : coeff_0;
    coeff_1 =
        (coord_int + 0 < 0 || coord_int + 0 >= max_input_coord) ? 0.0 : coeff_1;
    coeff_2 =
        (coord_int + 1 < 0 || coord_int + 1 >= max_input_coord) ? 0.0 : coeff_2;
    coeff_3 =
        (coord_int + 2 < 0 || coord_int + 2 >= max_input_coord) ? 0.0 : coeff_3;
...
__global__ void _ResizeBiCubicKernel(
  ...
  output_data[id] =
      y_info.coeff0_ * CubicInterpolationRowwise(image, x_int, y_int - 1,
                                                 input_height, input_width, w0,
                                                 w1, w2, w3) +
      y_info.coeff1_ * CubicInterpolationRowwise(image, x_int, y_int,
                                                 input_height, input_width, w0,
                                                 w1, w2, w3) +
      y_info.coeff2_ * CubicInterpolationRowwise(image, x_int, y_int + 1,
                                                 input_height, input_width, w0,
                                                 w1, w2, w3) +
      y_info.coeff3_ * CubicInterpolationRowwise(image, x_int, y_int + 2,
                                                 input_height, input_width, w0,
                                                 w1, w2, w3);
}
    ...
    case UpsampleMode::NN:
      return sizeof(int64_t) * output_dims.size() +
             sizeof(NearestMappingInfo) *
                 static_cast<size_t>(accumulate<const int64_t*, int64_t>(
                     output_dims.begin(), output_dims.end(), (int64_t)0));
    case UpsampleMode::LINEAR:
      return sizeof(LinearMappingInfo) *
             static_cast<size_t>(
                 accumulate<std::reverse_iterator<const int64_t*>, int64_t>(
                     output_dims.rbegin(), output_dims.rbegin() + 2,
                     (int64_t)0));
...
void ResizeNearestImpl(
    cudaStream_t stream, const int rank, TArray<int64_t>& input_shape,
    TArray<int64_t>& output_shape, TArray<int64_t>& input_strides,
    TArray<fast_divmod>& output_div_pitches, TArray<float>& scales_vals,
    TArray<float, 10>& roi_vals, const T* input_data, T* output_data,
    const size_t N, bool extrapolation_enabled, const T extrapolation_value,
    float cubic_coeff_a,
    ResizeCoordinateTransformationMode transform_coordinate,
    ResizeNearestMode calc_nearest_pixel, int64_t* /* prefix_dim_sum */,
    NearestMappingInfo* dims_mapping) {
  unsigned int blocksPerGrid = static_cast<unsigned int>(
      ceil(static_cast<float>(N) / GridDim::maxThreadsPerBlock));

  bool could2d =
      rank >= 2 &&
      transform_coordinate !=
          ResizeCoordinateTransformationMode::TF_CROP_AND_RESIZE &&
      std::all_of(scales_vals.Data(), scales_vals.Data() + (rank - 2),
                  [](float v) { return v == 1.0; });
  if (could2d) {
    int64_t output_height = output_shape[rank - 2];
    int64_t output_width = output_shape[rank - 1];
    fast_divmod div_output_image =
        (rank > 2)
            ? output_div_pitches[rank - 3]
            : fast_divmod(static_cast<int>(output_height * output_width));
    int blocksPerDimsMappingGrid =
        static_cast<int>(ceil((output_height + output_width) / 32.0));

    DISPATCH_RESIZE_COORDINATE_TRANSFORMATION_MODE(transform_coordinate, [&]() {
      DISPATCH_RESIZE_NEAREST_MODE(calc_nearest_pixel, [&]() {
        _ResizeNearestMappingKernel2D<T>
            <<<blocksPerDimsMappingGrid, 32, 0, stream>>>(
                static_cast<int>(input_shape[rank - 2]),
                ...
                extrapolation_enabled, coord_t(), nearest_t(), dims_mapping);
      });
    });
    if (extrapolation_enabled) {
      _ResizeNearestKernel2D<T, true>
          <<<blocksPerGrid, GridDim::maxThreadsPerBlock, 0, stream>>>(
              output_height, output_width,
              input_shape[rank - 2] * input_shape[rank - 1],
              static_cast<int>(input_shape[rank - 1]), div_output_image,
              output_div_pitches[rank - 2], input_data, output_data, N,
              extrapolation_value, dims_mapping);
    } else {
      _ResizeNearestKernel2D<T, false>
          <<<blocksPerGrid, GridDim::maxThreadsPerBlock, 0, stream>>>(
              output_height, output_width,
              input_shape[rank - 2] * input_shape[rank - 1],
              static_cast<int>(input_shape[rank - 1]), div_output_image,
  ...
  int64_t total_dim_sum = accumulate<int64_t*, int64_t>(
      output_shape.Data(), output_shape.Data() + rank, (int64_t)0);
  int blocksPerDimsMappingGrid =
      (int)(ceil(static_cast<double>(total_dim_sum) / 32));
  DISPATCH_RESIZE_COORDINATE_TRANSFORMATION_MODE(transform_coordinate, [&]() {
    DISPATCH_RESIZE_NEAREST_MODE(calc_nearest_pixel, [&]() {
      _ResizeNearestMappingKernel<T>
          <<<blocksPerDimsMappingGrid, 32, 0, stream>>>(
              rank, input_shape, output_shape, scales_vals, roi_vals,
              total_dim_sum, extrapolation_enabled, coord_t(), nearest_t(),
              reinterpret_cast<int64_t*>(dims_mapping),
              reinterpret_cast<NearestMappingInfo*>(
                  reinterpret_cast<int64_t*>(dims_mapping) + rank));
    });
  });
  _ResizeNearestKernel<T>
      <<<blocksPerGrid, GridDim::maxThreadsPerBlock, 0, stream>>>(
          rank, input_strides, output_div_pitches, input_data, output_data, N,
          extrapolation_value, reinterpret_cast<const int64_t*>(dims_mapping),
          reinterpret_cast<const NearestMappingInfo*>(
              reinterpret_cast<int64_t*>(dims_mapping) + rank));
  return;
}
...
void ResizeImpl(cudaStream_t stream, const UpsampleMode upsample_mode,
  ...
  bool isSame = std::all_of(scales_vals.Data(), scales_vals.Data() + rank,
                            [](float v) { return v == 1.0f; }) &&
                (coordinate_transform_mode !=
                 ResizeCoordinateTransformationMode::TF_CROP_AND_RESIZE);
  if (isSame) {
    assert(cudaMemcpyAsync(output_data, input_data, N * sizeof(T),
                           cudaMemcpyDeviceToDevice, stream) == 0);
    return;
  }

  if (upsample_mode == UpsampleMode::NN) {
    ResizeNearestImpl(stream, rank, input_shape, output_shape, input_strides,
                      output_div_pitches, scales_vals, roi_vals, input_data,
                      output_data, N, extrapolation_enabled,
                      extrapolation_value, cubic_coeff_a,
  ...
  // We support a special case of bilinear or bicubic if the input data is 4D
  // with the outer 2 scales being 1.0 We would have validated the outer scale
  // values by the time execution reaches this
  bool is_2D = (rank == 2 || rank == 4);

  // We support a special case of trilinear or tricubic if the input data is 5D
  // with the outer 2 scales being 1.0 We would have validated the outer scale
  // values by the time execution reaches this
  bool is_3D = (rank == 3 || rank == 5);

  // Should not hit this as we have already validated input rank/scales and we
  // provide verbose error messages to the user.
  ORT_ENFORCE(
      is_2D || is_3D,
  ...
  fast_divmod div_output_image;
  if (is_2D) {
    div_output_image = (rank > 2) ? output_div_pitches[rank - 3]
                                  : fast_divmod(gsl::narrow_cast<int>(N));
  } else if (is_3D) {
    div_output_image = (rank > 3) ? output_div_pitches[rank - 4]
                                  : fast_divmod(gsl::narrow_cast<int>(N));
  }
  ...
  int64_t output_height = output_shape[rank - 2];
  int64_t output_width = output_shape[rank - 1];
  int blocksPerDimsMappingGrid = static_cast<int>(
      ceil((output_depth + output_height + output_width) / 32.0));
    ...
    case UpsampleMode::LINEAR:
      if (is_2D) {
        DISPATCH_RESIZE_COORDINATE_TRANSFORMATION_MODE(
            coordinate_transform_mode, [&]() {
              _ResizeBilinearCoordinateMapping<T>
                  <<<blocksPerDimsMappingGrid, 32, 0, stream>>>(
                      input_shape[rank - 2], input_shape[rank - 1],
                      output_height, output_width, scales_vals[rank - 2],
                      scales_vals[rank - 1], roi_vals[rank - 2],
                      roi_vals[rank - 2 + rank], roi_vals[rank - 1],
                      roi_vals[rank - 1 + rank], output_height + output_width,
                      extrapolation_enabled, coord_t(),
                      reinterpret_cast<LinearMappingInfo*>(dims_mapping));
            });
        _ResizeBilinearKernel<T>
            <<<blocksPerGrid, GridDim::maxThreadsPerBlock, 0, stream>>>(
                input_shape[rank - 2], input_shape[rank - 1], output_height,
                output_width, output_div_pitches[rank - 2], div_output_image,
        ...
        DISPATCH_RESIZE_COORDINATE_TRANSFORMATION_MODE(
            coordinate_transform_mode, [&]() {
              _ResizeTrilinearCoordinateMapping<T>
                  <<<blocksPerDimsMappingGrid, 32, 0, stream>>>(
                      input_shape[rank - 3], input_shape[rank - 2],
                      input_shape[rank - 1], output_depth, output_height,
                      output_width, scales_vals[rank - 3],
                      scales_vals[rank - 2], scales_vals[rank - 1],
        ...
        _ResizeTrilinearKernel<T>
            <<<blocksPerGrid, GridDim::maxThreadsPerBlock, 0, stream>>>(
                input_shape[rank - 3], input_shape[rank - 2],
                input_shape[rank - 1], output_depth, output_height,
                output_width, output_div_pitches[rank - 3],
                output_div_pitches[rank - 2], div_output_image, input_data,
                output_data, N, extrapolation_value,
                reinterpret_cast<LinearMappingInfo*>(dims_mapping));
        return;
      }
      LOG(ERROR) << "Only bilinear/trilinear and bicubic modes are supported "
                    "in Resize";
      assert(false);
      break;
    case UpsampleMode::CUBIC:
      if (is_2D) {
        DISPATCH_RESIZE_COORDINATE_TRANSFORMATION_MODE(
            coordinate_transform_mode, [&]() {
              _ResizeCubicCoordinateMapping<T>
                  <<<blocksPerDimsMappingGrid, 32, 0, stream>>>(
                      input_shape[rank - 2], input_shape[rank - 1],
                      output_height, output_width, scales_vals[rank - 2],
                      scales_vals[rank - 1], roi_vals[rank - 2],
                      roi_vals[rank - 2 + rank], roi_vals[rank - 1],
                      roi_vals[rank - 1 + rank], output_height + output_width,
                      extrapolation_enabled, cubic_coeff_a, exclude_outside,
                      coord_t(),
                      reinterpret_cast<CubicMappingInfo*>(dims_mapping));
            });
        _ResizeBiCubicKernel<T>
            <<<blocksPerGrid, GridDim::maxThreadsPerBlock, 0, stream>>>(
                input_shape[rank - 2], input_shape[rank - 1], output_height,
                output_width, output_div_pitches[rank - 2], div_output_image,
                input_data, output_data, N, extrapolation_value,
...
#define SPECIALIZED_IMPL(T)                                                    \
  template void ResizeImpl<T>(                                                 \
      cudaStream_t stream, const UpsampleMode upsample_mode, const int rank,   \

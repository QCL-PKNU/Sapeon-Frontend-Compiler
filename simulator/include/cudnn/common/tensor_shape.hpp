#ifndef CUDNN_COMMON_TENSOR_SHAPE_HPP
#define CUDNN_COMMON_TENSOR_SHAPE_HPP
...
class TensorShape {
 ...
 public:
  ...
  TensorShape(std::initializer_list<int64_t> dims)
      : TensorShape(gsl::make_span(dims.begin(), dims.end())) {}
  TensorShape(const int64_t* dimension_sizes, size_t dimension_count)
      : TensorShape(
            gsl::span<const int64_t>(dimension_sizes, dimension_count)) {}
  TensorShape(const std::vector<int64_t>& dims, size_t start, size_t end)
      : TensorShape(gsl::span<const int64_t>(&dims[start], end - start)) {}

  // Create a TensorShape that points to an existing buffer internally. As no
  // copy is made, 'data' must remain valid for the life of the TensorShape
  static const TensorShape FromExistingBuffer(
      const std::vector<int64_t>& data) {
    return TensorShape(
        External{},
        gsl::span<int64_t>(const_cast<int64_t*>(data.data()), data.size()));
  }
  ...
  */
  ...
  bool operator==(const TensorShape& other) const noexcept {
    return GetDims() == other.GetDims();
  }
  bool operator!=(const TensorShape& other) const noexcept {
    return !(*this == other);
  }

  size_t NumDimensions() const noexcept { return values_.size(); }

  /**
     Copy dims into an array with given size
  */
  void CopyDims(int64_t* dims, size_t num_dims) const {
    memcpy(dims, values_.data(),
           sizeof(int64_t) * std::min(num_dims, NumDimensions()));
  }

  /**
     Copy dims from a specific start dim into an array with given size
     `start_dim` is expected to be in the inclusive range [0, NumDimensions() -
     1] and this function does no checks to ensure that
  */
  void CopyDims(int64_t* dims, size_t start_dim, size_t num_dims) const {
    memcpy(dims, values_.data() + start_dim,
           sizeof(int64_t) * std::min(num_dims, NumDimensions() - start_dim));
  }
  ...
  */
  ...
  TensorShapeVector AsShapeVector() const { return ToShapeVector(values_); }
  ...
  /**
     ...
     @param dimension Return size up to this dimension. Value must be between 0
     and this->NumDimensions(), inclusive.
  */
  int64_t SizeToDimension(size_t dimension) const;

  /**
     Return the total number of elements from the specified dimension to the end
     of the tensor shape. If the dimension interval is empty (dimension ==
     this->NumDimensions()), return 1.
     @param dimension Return size from this dimension to the end. Value must be
     between 0 and this->NumDimensions(), inclusive.
  */
  ...
  */
  TensorShape Slice(size_t dimstart) const {
    return Slice(dimstart, values_.size());
  }
  ...
  */
  ...
  /**
     ...
     Assumes start and end are between 0 and this->NumDimensions(), inclusive,
     and that start < end.
  */
 ...
 private:
  ...
  friend struct ProviderHostImpl;  // So that the shared provider interface can
                                   // access Allocate
};
#ifdef __GNUC__
...
#endif  // CUDNN_COMMON_TENSOR_SHAPE_HPP

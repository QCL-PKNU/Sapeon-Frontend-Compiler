...
#ifndef CUDNN_COMMON_CUDNN_COMMON_HPP
#define CUDNN_COMMON_CUDNN_COMMON_HPP
...
class CudnnTensor final {
 public:
  ...
  bool Set(gsl::span<const int64_t> input_dims, cudnnDataType_t dataType);
  bool Set(const CudnnTensor &x_desc, cudnnBatchNormMode_t mode);
...
class CudnnDataTensor final {
 public:
  ...
  bool Set(cudnnDataType_t dataType, int64_t max_seq_length, int64_t batch_size,
           int64_t data_size, const int32_t *seq_lengths);
...
class CudnnFilterDescriptor final {
 public:
  ...
  bool Set(gsl::span<const int64_t> filter_dims, cudnnDataType_t data_typ);
...
class CudnnDropout final {
 public:
  ...
  bool GetCudnnDropoutStatesSize(const cudnnHandle_t &cudnnHandle,
                                 size_t &stateSize) {
    assert(cudnnDropoutGetStatesSize(cudnnHandle, &stateSize) ==
           CUDNN_STATUS_SUCCESS);

    return true;
  }

  bool Set(const cudnnHandle_t &cudnnHandle, void *states, size_t stateSize,
           float dropout = 0.0f, unsigned long long seed = 1) {
    assert(CreateDescriptorIfNeeded());
    assert(cudnnSetDropoutDescriptor(dropout_desc_, cudnnHandle, dropout,
                                     states, stateSize,
                                     seed) == CUDNN_STATUS_SUCCESS);
  ...
  operator cudnnDropoutDescriptor_t() const { return dropout_desc_; }

  bool CreateDescriptorIfNeeded() {
    if (!dropout_desc_)
      assert(cudnnCreateDropoutDescriptor(&dropout_desc_) ==
             CUDNN_STATUS_SUCCESS);
    return true;
  }
...
inline cudnnStatus_t BatchNormalizationForwardInferenceHelper(
...
inline cudnnStatus_t BatchNormalizationForwardTrainingHelper(
    cudnnHandle_t handle, cudnnBatchNormMode_t mode, const void *alpha,
    const void *beta, const cudnnTensorDescriptor_t xDesc, const void *x,
    const cudnnTensorDescriptor_t yDesc, void *y,
    const cudnnTensorDescriptor_t bnScaleBiasMeanVarDesc, const void *bnScale,
...
inline cudnnStatus_t LRNCrossChannelForwardHelper(
  ...
  return cudnnLRNCrossChannelForward(handle, normDesc, lrnMode, alpha, xDesc, x,
                                     beta, yDesc, y);
}

inline cudnnStatus_t SetLRNDescriptorHelper(cudnnLRNDescriptor_t normDesc,
                                            unsigned lrnN, double lrnAlpha,
                                            double lrnBeta, double lrnK) {
  return cudnnSetLRNDescriptor(normDesc, lrnN, lrnAlpha, lrnBeta, lrnK);
}

inline cudnnStatus_t PoolingForwardHelper(
    cudnnHandle_t handle, const cudnnPoolingDescriptor_t poolingDesc,
    const void *alpha, const cudnnTensorDescriptor_t xDesc, const void *x,
    const void *beta, const cudnnTensorDescriptor_t yDesc, void *y) {
  return cudnnPoolingForward(handle, poolingDesc, alpha, xDesc, x, beta, yDesc,
                             y);
}

inline cudnnStatus_t SetPoolingNdDescriptorHelper(
  ...
  return cudnnSetPoolingNdDescriptor(poolingDesc, mode, maxpoolingNanOpt,
                                     nbDims, windowDimA, paddingA, strideA);
}

#define CUDNN_RETURN_IF_ERROR(X) assert(X == CUDNN_STATUS_SUCCESS)

}  // namespace Cudnn

#endif  // CUDNN_COMMON_CUDNN_COMMON_HPP

...
#ifndef CUDNN_COMMON_GSL_LITE_HPP
#define CUDNN_COMMON_GSL_LITE_HPP
...
#pragma message( \
    "gsl_CONFIG_ALLOWS_SPAN_CONTAINER_CTOR is deprecated since gsl-lite 0.7.0; replace with gsl_CONFIG_ALLOWS_UNCONSTRAINED_SPAN_CONTAINER_CTOR, or consider span(with_container, cont).")
#endif

#if defined(gsl_CONFIG_CONTRACT_LEVEL_EXPECTS_ONLY)
#pragma message( \
    "gsl_CONFIG_CONTRACT_LEVEL_EXPECTS_ONLY is deprecated since gsl-lite 0.35.0; replace with gsl_CONFIG_CONTRACT_LEVEL_ON and gsl_CONFIG_CONTRACT_EXPECTS_ONLY.")
...
#pragma message( \
    "gsl_CONFIG_CONTRACT_LEVEL_ENSURES_ONLY is deprecated since gsl-lite 0.35.0; replace with gsl_CONFIG_CONTRACT_LEVEL_ON and gsl_CONFIG_CONTRACT_ENSURES_ONLY.")
#define gsl_CONFIG_CONTRACT_LEVEL_ON
...
#if 2 <= defined(gsl_CONFIG_CONTRACT_LEVEL_AUDIT) +      \
             defined(gsl_CONFIG_CONTRACT_LEVEL_ON) +     \
             defined(gsl_CONFIG_CONTRACT_LEVEL_ASSUME) + \
             defined(gsl_CONFIG_CONTRACT_LEVEL_OFF)
...
#if 2 <= defined(gsl_CONFIG_CONTRACT_VIOLATION_THROWS) +         \
...
#if defined(gsl_CONFIG_CONTRACT_LEVEL_ASSUME) &&          \
    (defined(gsl_CONFIG_CONTRACT_VIOLATION_THROWS) ||     \
     defined(gsl_CONFIG_CONTRACT_VIOLATION_TERMINATES) || \
     defined(gsl_CONFIG_CONTRACT_VIOLATION_CALLS_HANDLER))
// `gsl_CONFIG_CONTRACT_LEVEL_ASSUME` should not be combined with any of the
// violation response macros. Contract violations are undefined behavior in
// ASSUME mode, and code which expects a particular violation response will not
// work as expected.
...
#define gsl_REQUIRES_R(R, VA) typename std::enable_if<VA, R>::type

#define gsl_REQUIRES_A(VA) , typename std::enable_if<VA, void*>::type = nullptr
...
#define gsl_HAVE_DEDUCTION_GUIDES \
  (gsl_CPP17_000 && !gsl_BETWEEN(gsl_COMPILER_MSVC_VERSION, 1, 999))
...
gsl_DISABLE_MSVC_WARNINGS(
    26410 26415 26418 26472 26439 26440 26473 26481 26482 26446 26490 26487)
  ...
  template <class E>
  inline gsl_constexpr auto data(std::initializer_list<E> il)
      gsl_noexcept->E const* {
    return il.begin();
  }
  ...
  struct is_compatible_element<
      C, E, std17::void_t<decltype(std17::data(std::declval<C>()))> >
...
#if gsl_HAVE(TYPE_TRAITS)
  template <class T gsl_REQUIRES_T(std::is_pointer<T>::value)>
  using owner = T;
#else
  template <class T>
  using owner = T;
...
#define gsl_ELIDE_CONTRACT(x)                                      \
  static_assert(::std::is_convertible<decltype((x)), bool>::value, \
...
#define gsl_ASSUME(x) \
  gsl_ELIDE_CONTRACT(x) /* there is no assume intrinsic in CUDA device code */
...
#else
#define gsl_ASSUME(x)                                                        \
  gsl_ELIDE_CONTRACT(x) /* unknown compiler; cannot rely on assume intrinsic \
                         */
#endif

#define gsl_ELIDE_CONTRACT_EXPECTS \
  (0 == (gsl_CONFIG_CONTRACT_LEVEL_MASK & 0x01))
#define gsl_ELIDE_CONTRACT_ENSURES \
  (0 == (gsl_CONFIG_CONTRACT_LEVEL_MASK & 0x10))
#define gsl_ASSUME_CONTRACT_EXPECTS \
  (0 != (gsl_CONFIG_CONTRACT_LEVEL_MASK & 0x04))
#define gsl_ASSUME_CONTRACT_ENSURES \
  (0 != (gsl_CONFIG_CONTRACT_LEVEL_MASK & 0x40))
#define gsl_ELIDE_CONTRACT_EXPECTS_AUDIT \
  (0 == (gsl_CONFIG_CONTRACT_LEVEL_MASK & 0x02))
#define gsl_ELIDE_CONTRACT_ENSURES_AUDIT \
  (0 == (gsl_CONFIG_CONTRACT_LEVEL_MASK & 0x20))
...
#define Expects(x)                                                      \
  ::gsl::fail_fast_assert((x), "GSL: Precondition failure at " __FILE__ \
                               ":" gsl_STRINGIFY(__LINE__))
#elif gsl_CONFIG(CONTRACT_VIOLATION_CALLS_HANDLER_V)
#define Expects(x)                                                        \
  ::gsl::fail_fast_assert((x), #x, "GSL: Precondition failure", __FILE__, \
                          __LINE__)
#else
...
#elif gsl_CONFIG(CONTRACT_VIOLATION_THROWS_V)
#define Ensures(x)                                                       \
  ::gsl::fail_fast_assert((x), "GSL: Postcondition failure at " __FILE__ \
                               ":" gsl_STRINGIFY(__LINE__))
#elif gsl_CONFIG(CONTRACT_VIOLATION_CALLS_HANDLER_V)
#define Ensures(x)                                                         \
  ::gsl::fail_fast_assert((x), #x, "GSL: Postcondition failure", __FILE__, \
                          __LINE__)
#else
...
#define gsl_STRINGIFY_(x) #x
  ...
  gsl_api inline void fail_fast_assert(bool cond, char const* const expression,
                                       char const* const message,
                                       char const* const file, int line) {
#ifdef __CUDA_ARCH__
    assert(cond);
#else   // __CUDA_ARCH__
    if (!cond) fail_fast_assert_handler(expression, message, file, line);
#endif  // __CUDA_ARCH__
  }
  ...
  inline unsigned char uncaught_exceptions() gsl_noexcept {
    return detail::to_uchar(
        *reinterpret_cast<unsigned*>(__cxa_get_globals() + sizeof(void*)));
  }
#endif
  }  // namespace std11
#endif
  ...
  class final_action_return : public final_action<F> {
   public:
    ...
    gsl_api ~final_action_return() override {
      if (std11::uncaught_exceptions() != exception_count) this->dismiss();
    }

    gsl_is_delete_access : gsl_api final_action_return(
                               final_action_return const&) gsl_is_delete;
  ...
  template <class F>
  gsl_api inline final_action_return<F> on_return(F const& action)
      gsl_noexcept {
  ...
  class final_action_error : public final_action<F> {
   public:
    ...
    gsl_is_delete_access : gsl_api final_action_error(final_action_error const&)
                               gsl_is_delete;
  ...
  class final_action {
   public:
    ...
    gsl_api final_action(Action action) : action_(action), invoke_(true) {}
   ...
   protected:
    gsl_api void dismiss() { invoke_ = false; }
...
#pragma warning(pop)
  ...
  gsl_api inline gsl_constexpr14 typename Container::value_type& at(
  ...
  template <class Container>
  gsl_api inline gsl_constexpr14 typename Container::value_type const& at(
  ...
  template <class T>
  gsl_api inline const gsl_constexpr14 T at(std::initializer_list<T> cont,
                                            size_t pos) {
    Expects(pos < cont.size());
...
#undef gsl_not_null_explicit
...
#if gsl_HAVE(RVALUE_REFERENCE)
    gsl_api gsl_constexpr not_null(not_null&& other)
    ...
    gsl_api gsl_constexpr14 get_result_t get() const {
      // Without cheating and changing ptr_ from the outside, this check is
      // superfluous:
  ...
  template <class T, class U>
  gsl_api inline gsl_constexpr bool operator==(not_null<T> const& l,
                                               not_null<U> const& r) {
    return l.get() == r.get();
  }
...
#if gsl_HAVE(DEFAULT_FUNCTION_TEMPLATE_ARG)
#define gsl_ENABLE_IF_INTEGRAL_T(T) gsl_REQUIRES_T((std::is_integral<T>::value))
#else
#define gsl_ENABLE_IF_INTEGRAL_T(T)
  ...
  class span {
   ...
   public:
...
#if gsl_HAVE(IS_DELETE)
    gsl_api gsl_constexpr span(reference data_in) : span(&data_in, 1) {}
    ...
    template <size_t N
#if gsl_HAVE(DEFAULT_FUNCTION_TEMPLATE_ARG)
                  gsl_REQUIRES_T((std::is_convertible<
    ...
    template <class Container gsl_REQUIRES_T(
...
#if gsl_HAVE(DEFAULT_FUNCTION_TEMPLATE_ARG)
    template <
        class ArrayElementType = typename std::add_pointer<element_type>::type>
#else
    template <class ArrayElementType>
#endif
    gsl_api gsl_constexpr span(unique_ptr<ArrayElementType> const& ptr,
                               index_type count)
    ...
    gsl_api gsl_constexpr14 span subspan(index_type offset,
                                         index_type count) const gsl_noexcept {
    ...
    gsl_api gsl_constexpr pointer data() const gsl_noexcept { return first_; }
  ...
  template <class T, class U>
  gsl_api inline gsl_constexpr bool operator==(span<T> const& l,
                                               span<U> const& r) {
    return l.size() == r.size() && (l.begin() == r.begin() ||
  ...
  template <class T, size_t N>
  gsl_api inline gsl_constexpr span<T> make_span(std::array<T, N> & arr) {
    return span<T>(arr);
  }

  template <class T, size_t N>
  gsl_api inline gsl_constexpr span<const T> make_span(
  ...
  gsl_api inline gsl_constexpr auto make_span(Container & cont)
  ...
  template <class Container,
            class EP = decltype(std17::data(std::declval<Container&>()))>
  gsl_api inline gsl_constexpr auto make_span(Container const& cont)
  ...
  template <class Container>
  gsl_api inline gsl_constexpr span<typename Container::value_type> make_span(
  ...
  gsl_api inline gsl_constexpr14 std::size_t string_length(T* ptr,
                                                           std::size_t max) {
    if (ptr == gsl_nullptr || max <= 0) return 0;
  ...
  class basic_string_span {
   public:
    ...
    template <std::size_t N>
    gsl_api gsl_constexpr basic_string_span(
        std::array<typename std11::remove_const<element_type>::type, N> const&
            arr)
        : span_(remove_z(arr)) {}
    ...
    gsl_api gsl_constexpr basic_string_span(Container& cont) : span_((cont)) {}
...
#else
    gsl_constexpr basic_string_span(basic_string_span const& rhs)
        gsl_noexcept = default;

    gsl_constexpr basic_string_span(basic_string_span&& rhs)
        gsl_noexcept = default;
#endif
#endif
...
#if gsl_HAVE(DEFAULT_FUNCTION_TEMPLATE_ARG)
                  gsl_REQUIRES_T((std::is_convertible<
                                  typename basic_string_span<U>::pointer,
                                  pointer>::value))
#endif
              >
...
#if gsl_CPP11_OR_GREATER || gsl_COMPILER_MSVC_VERSION >= 120
    template <class U gsl_REQUIRES_T(
    ...
    basic_string_span& operator=(basic_string_span const& rhs)
        gsl_noexcept = default;

    basic_string_span& operator=(basic_string_span&& rhs)
        gsl_noexcept = default;
#endif
    ...
    gsl_api gsl_constexpr basic_string_span subspan(index_type offset,
    ...
    gsl_api iterator begin() const gsl_noexcept { return span_.begin(); }

    gsl_api iterator end() const gsl_noexcept { return span_.end(); }

    gsl_api reverse_iterator rbegin() const gsl_noexcept {
      return span_.rbegin();
    }

    gsl_api reverse_iterator rend() const gsl_noexcept { return span_.rend(); }
    ...
    gsl_api const_iterator cend() const gsl_noexcept { return span_.cend(); }
...
#if gsl_HAVE(ARRAY)
    template <size_t N>
    gsl_api static gsl_constexpr14 span_type remove_z(
        std::array<typename std11::remove_const<element_type>::type, N>& arr) {
  ...
  template <class T, class U>
  gsl_api inline gsl_constexpr14 bool operator==(basic_string_span<T> const& l,
                                                 U const& u) gsl_noexcept {
  ...
  template <class T,
            class U gsl_REQUIRES_T((!detail::is_basic_string_span<U>::value))>
  gsl_api inline gsl_constexpr14 bool operator==(
...
// Disable stream support since Apple std lib is broken
  ...
  gsl_api static span<T> ensure_sentinel(
      T* seq, SizeType max = (std::numeric_limits<SizeType>::max)()) {
    typedef T* pointer;

    gsl_SUPPRESS_MSVC_WARNING(26429,
                              "f.23: symbol 'cur' is never tested for "
                              "nullness, it can be marked as not_null")

        pointer cur = seq;

    while (static_cast<SizeType>(cur - seq) < max && *cur != Sentinel) ++cur;
  ...
  template <class Container>
  gsl_api inline span<
      typename std::remove_pointer<typename Container::pointer>::type>
  ...
  class basic_zstring_span {
   public:
    ...
    gsl_constexpr14 basic_zstring_span& operator=(
        basic_zstring_span const& other) = default;
    gsl_constexpr14 basic_zstring_span& operator=(basic_zstring_span&& other) =
        default;
#else
    gsl_api gsl_constexpr basic_zstring_span(basic_zstring_span const& other)
        : span_(other.span_) {}
    gsl_api gsl_constexpr basic_zstring_span& operator=(
        basic_zstring_span const& other) {
      span_ = other.span_;
      return *this;
    }
#endif
...
#endif  // CUDNN_COMMON_GSL_LITE_HPP

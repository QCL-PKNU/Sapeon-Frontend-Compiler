...
void SCOPE::InitOutputTensor() {
  ...
  if (inputs_count_ > 1) {
    ...
    for (size_t index = 1; index < inputs_count_; index++) {
      ...
      ComputeOutputShape("Mean", in_vector1, in_vector2, out_vector);
...
void SCOPE::AllocateMemory() {
  if (inputs_count_ > 1) {
    ...
    cudaMalloc(&data_temp_, output_->size());
  }
}
...
void SCOPE::OperationForward() {
  ...
  Type div_count = (Type)inputs_count_;
  Fill(stream, data_temp_, div_count, count);
  ...
  if (index == inputs_count_) {  // broadcast 가 필요 없음
    if (inputs_count_ == 2) {
      ...
      BinaryElementwiseBroadcastPrepare(out_vector, out_vector, out_vector,
                                        output_rank_or_simple_broadcast, fdm_H,
                                        fdm_C, lhs_padded_strides,
                                        rhs_padded_strides, fdm_output_strides);

      Impl_Div<Type>(stream, output_rank_or_simple_broadcast,
                     &lhs_padded_strides, data_output_, &rhs_padded_strides,
                     data_temp_, &fdm_output_strides, fdm_H, fdm_C,
                     data_output_, count);
      return;
    }
  ...
  if (inputs_count_ == 2) {
    ...
    Impl_Div<Type>(stream, output_rank_or_simple_broadcast, &lhs_padded_strides,
                   data_output_, &rhs_padded_strides, data_temp_,
  ...
  GeneralImplDispatchTarget<Type, DataType>(stream, inputs_count_, data_input_,
                                            data_output_, inputs_, output_);
  BinaryElementwiseBroadcastPrepare(
      out_vector, out_vector, out_vector, output_rank_or_simple_broadcast,

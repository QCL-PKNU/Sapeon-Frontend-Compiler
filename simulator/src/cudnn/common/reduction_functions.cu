...
#include "cudnn/common/atomic_common.cuh"
#include "cudnn/common/common.cuh"
#include "cudnn/common/cuda_utils.hpp"
#include "cudnn/common/reduction_functions.hpp"
#include "cudnn/common/reduction_utils.cuh"
#include "cudnn/common/unary_elementwise_impl.cuh"
...
dim3 compute_block_dim(int num_cols) {
  ...
  const int y =
      std::min(MAX_NUM_WARPS_PER_BLOCK,
               std::max(1, num_cols / (MAX_NUM_ELEMENTS_PER_THREAD * x)));
  return dim3(x, y);
}
...
std::pair<dim3, dim3> compute_grid_and_block_dims(int num_rows, int num_cols) {
  ...
  const auto grid_x =
      std::min<int>(MAX_NUM_BLOCKS_IN_GRID_ROW,
                    std::max<int>(1, num_cols / (MAX_NUM_ELEMENTS_PER_THREAD *
 ...
 *
 * Given buffer element type TBuf, the intermediate buffer layout looks like
 * this:
 *
 * -----
...
size_t compute_reduce_matrix_columns_intermediate_buffer_size(int element_size,
                                                              int num_rows,
                                                              int num_cols) {
  assert(element_size >= 0 && num_rows >= 0 && num_cols >= 0);
...
bool get_reduction_buffers(int num_rows, int num_cols, void* buffer,
                           size_t buffer_size, TBuf*& block_reductions_buffer,
  ...
  const uintptr_t block_done_counts_buffer_addr = round_up_to_aligned(
      block_reductions_addr +
          static_cast<size_t>(num_rows) * grid_dim.x * sizeof(TBuf),
      alignof(int));
  const uintptr_t end_addr = block_done_counts_buffer_addr +
  ...
  block_done_counts_buffer =
      reinterpret_cast<int*>(block_done_counts_buffer_addr);

  return true;
}

template <typename TIn, typename TOut, typename TBuf, typename TOp,
          typename TFinalOp, bool DivideResultBySize>
__device__ void reduce_all(const int num_elements, const TIn* const input,
                           TOut* const output,
                           TBuf* const block_reductions_buffer,
                           int* const block_done_count_buffer) {
  ...
  const int tid_in_grid_row =
      bid_in_grid_row * (blockDim.x * blockDim.y) + tid_in_block;
  ...
  const int num_threads_in_grid_row =
      num_blocks_in_grid_row * num_threads_in_block;
  ...
  // One thread reduces MAX_NUM_ELEMENTS_PER_THREAD elements to a thread
  // register in one iteration.
  TBuf value = 0;
...
#pragma unroll
  ...
  // Given a fixed amount of threads, we prefer threads over warps over blocks
  // so that we never have cases such as
...
#pragma unroll
  ...
  // Only the block which saw that count equals to num_blocks_in_grid_row - 1
  // can enter the following block.
  if (is_last_block_done) {
    ...
    for (int stride = pow2_bound / 2; stride > 0; stride /= 2) {
      if (tid_in_block < stride &&
          tid_in_block + stride < num_blocks_in_grid_row) {
        block_reductions_buffer[tid_in_block] +=
            block_reductions_buffer[tid_in_block + stride];
      }
      __syncthreads();
    }
...
__global__ void reduce_matrix_columns_kernel(
    const int num_rows, const int num_cols, const TIn* const input,
    TOut* const output, TBuf* const block_reductions_buffer,
    int* const block_done_counts_buffer) {
  const int num_blocks_in_grid_row = gridDim.x;
  ...
  for (int64_t row_id = row_id_in_grid; row_id < num_rows;
       row_id += num_grid_rows) {
    const TIn* const row_data = input + row_id * num_cols;
    TOut* const row_output = output + row_id;
    TBuf* const row_block_reductions_buffer =
        block_reductions_buffer + row_id * num_blocks_in_grid_row;
    ...
    reduce_all<TIn, TOut, TBuf, TOp, TFinalOp, DivideResultBySize>(
        num_cols, row_data, row_output, row_block_reductions_buffer,
        row_block_done_counts_buffer);
  }
}

template <typename TIn, typename TOut, typename TOp, typename TFinalOp,
          bool DivideResultBySize>
bool call_reduce_matrix_columns(cudaStream_t stream, const TIn* input,
                                TOut* output, const int num_rows,
                                const int num_cols, void* buffer,
                                size_t buffer_size) {
  assert(num_rows >= 0 && num_cols >= 0);

  using TBuf = AccumulationType_t<TIn>;

  const auto grid_and_block_dims =
      compute_grid_and_block_dims(num_rows, num_cols);
  const dim3& grid_dim = grid_and_block_dims.first;
  const dim3& block_dim = grid_and_block_dims.second;

  TBuf* block_reductions_buffer;
  int* block_done_counts_buffer;
  assert(get_reduction_buffers(num_rows, num_cols, buffer, buffer_size,
                               block_reductions_buffer,
                               block_done_counts_buffer));

  // If more than one block is used per grid row, then inter-block reduction is
  // needed.
  if (grid_dim.x > 1) {
    assert(cudaMemsetAsync(block_done_counts_buffer, 0, num_rows * sizeof(int),
                           stream) == 0);
  }

  const int shared_mem_size =
      sizeof(TBuf) * block_dim.x * block_dim.y / GPU_WARP_SIZE_HOST;
  reduce_matrix_columns_kernel<TIn, TOut, TBuf, TOp, TFinalOp,
                               DivideResultBySize>
      <<<grid_dim, block_dim, shared_mem_size, stream>>>(
          num_rows, num_cols, input, output, block_reductions_buffer,
          block_done_counts_buffer);
...
bool reduce_sum(cudaStream_t stream, const TIn* input, TOut* output, int size,
                void* buffer, size_t buffer_size) {
  return detail::call_reduce_matrix_columns<TIn, TOut, Identity, Identity,
                                            false>(stream, input, output, 1,
                                                   size, buffer, buffer_size);
}

template <typename TIn, typename TOut>
bool reduce_square_sum(cudaStream_t stream, const TIn* input, TOut* output,
                       int size, void* buffer, size_t buffer_size) {
  return detail::call_reduce_matrix_columns<TIn, TOut, Square, Identity, false>(
      stream, input, output, 1, size, buffer, buffer_size);
}

template <typename TIn, typename TOut>
bool reduce_l2_norm(cudaStream_t stream, const TIn* input, TOut* output,
                    int size, void* buffer, size_t buffer_size) {
...
template <typename TIn, typename TOut>
bool reduce_mean(cudaStream_t stream, const TIn* input, TOut* output, int size,
                 void* buffer, size_t buffer_size) {
  return detail::call_reduce_matrix_columns<TIn, TOut, Identity, Identity,
                                            true>(stream, input, output, 1,
                                                  size, buffer, buffer_size);
}

#define INSTANTIATE_REDUCE_SUM(TIn, TOut)                                    \
  template bool reduce_sum<TIn, TOut>(cudaStream_t stream, const TIn* input, \
                                      TOut* output, int size, void* buffer,  \
                                      size_t buffer_size)
...
#define INSTANTIATE_REDUCE_SQUARE_SUM(TIn, TOut)                     \
  template bool reduce_square_sum<TIn, TOut>(                        \
      cudaStream_t stream, const TIn* input, TOut* output, int size, \
      void* buffer, size_t buffer_size)
...
#define INSTANTIATE_REDUCE_L2_NORM(TIn, TOut)                        \
  template bool reduce_l2_norm<TIn, TOut>(                           \
      cudaStream_t stream, const TIn* input, TOut* output, int size, \
      void* buffer, size_t buffer_size)
...
#define INSTANTIATE_REDUCE_MEAN(TIn, TOut)                                    \
  template bool reduce_mean<TIn, TOut>(cudaStream_t stream, const TIn* input, \
                                       TOut* output, int size, void* buffer,  \
                                       size_t buffer_size)
...
__global__ void reduce_matrix_rows_kernel(const TIn* input, TOut* output, int m,
                                          int n) {
  ...
  for (int col = tid_x_in_grid; col < n; col += x_grid_stride) {
    ...
    TBuf sum = TBuf(0.0f);
    // This loops load multiple blockDim.y-by-blockDim.x sub-tensors from the
    // input.
    for (int row = tid_y_in_grid; row < m; row += y_grid_stride) {
      // Thread-level reduction. Each thread loads y_load_count_per_thread
      // values and aggregrate them.
#pragma unroll(y_load_count_per_thread)
      for (int row_inner = 0; row_inner < y_load_count_per_thread;
           ++row_inner) {
...
#pragma unroll(4)
    for (int stride = blockDim.y / 2; stride > 0; stride /= 2) {
      if (threadIdx.y < stride) {
        shared_memory[tid_in_block] +=
            shared_memory[tid_in_block + stride * blockDim.x];
      }
      __syncthreads();
    }
...
template <typename TIn, typename TOut, typename TBuf>
bool call_reduce_matrix_rows(cudaStream_t stream, const TIn* input,
                             TOut* output, int m, int n,
                             bool reset_initial_output) {
  assert(m >= 0 && n >= 0);

  if (reset_initial_output) {
    assert(cudaMemsetAsync(output, 0, n * sizeof(TOut), stream) == 0);
  }
  ...
  const int block_x_dim =
      least_pow2_bound(std::max(1, std::min(n, GPU_WARP_SIZE_HOST)));
  const int block_y_dim = least_pow2_bound(
      std::max(1, std::min(max_num_threads_in_block / block_x_dim,
                           m / load_count_per_thread)));
  const int grid_x_dim =
      std::max(1, std::min(n / block_x_dim, max_num_blocks_in_grid));
  const int grid_y_dim = std::max(
      1, std::min(max_num_blocks_in_grid / grid_x_dim, m / block_y_dim / 4));

  const dim3 grid(grid_x_dim, grid_y_dim, 1);
  const dim3 block(block_x_dim, block_y_dim, 1);

  reduce_matrix_rows_kernel<TIn, TOut, TBuf>
      <<<grid, block, block.y * block.x * sizeof(TBuf), stream>>>(input, output,
                                                                  m, n);
...
template <typename T>
struct OP_Div {
  __device__ __inline__ T operator()(const T& a) const { return a / v_; }
...
template <typename T>
void UnaryDiv(cudaStream_t stream, const T* input, T* output, T denominator,
              size_t count) {
  UnaryElementWiseImpl(stream, input, output, OP_Div<T>(denominator), count);
}

#define INSTANTIATE_UNARY_DIV(T)                                            \
  template void UnaryDiv<T>(cudaStream_t stream, const T* input, T* output, \
                            T denominator, size_t count)
...
template <typename TIn, typename TOut>
bool reduce_matrix_rows(cudaStream_t stream, const TIn* input, TOut* output,
                        int m, int n, bool reset_initial_output) {
  using TBuf = AccumulationType_t<TIn>;
  return detail::call_reduce_matrix_rows<TIn, TOut, TBuf>(
      stream, input, output, m, n, reset_initial_output);
}

#define INSTANTIATE_REDUCE_MATRIX_ROWS(T)                                     \
  template bool reduce_matrix_rows<T, T>(cudaStream_t stream, const T* input, \
                                         T* output, int m, int n,             \
                                         bool reset_initial_output)
...
template <typename TIn, typename TOut>
bool reduce_matrix_columns(cudaStream_t stream, const TIn* input, TOut* output,
                           int m, int n, void* buffer, size_t buffer_size) {
  return detail::call_reduce_matrix_columns<TIn, TOut, Identity, Identity,
                                            false>(stream, input, output, m, n,
                                                   buffer, buffer_size);
}

#define INSTANTIATE_REDUCE_MATRIX_COLUMNS(T)                        \
  template bool reduce_matrix_columns<T, T>(                        \
      cudaStream_t stream, const T* input, T* output, int m, int n, \
      void* buffer, size_t buffer_size)

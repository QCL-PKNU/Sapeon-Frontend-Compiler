#ifndef CUDNN_COMMON_MATMUL_HELPER_HPP
#define CUDNN_COMMON_MATMUL_HELPER_HPP
...
namespace Cudnn {
template <typename T>
inline void TensorShapeCopyDims(const TensorShape& shape, T* dims,
                                size_t num_dims) {
  size_t n = std::min(num_dims, shape.NumDimensions());
  for (size_t i = 0; i != n; ++i) dims[i] = static_cast<ptrdiff_t>(shape[i]);
}

template <>
inline void TensorShapeCopyDims(const TensorShape& shape, int64_t* dims,
                                size_t num_dims) {
...
class MatMulComputeHelper {
 public:
  ...
  // For CUDA/ROCM kernel when we can use GemmStridedBatched, we don't need to
  // fill the offsets.
  bool Compute(const TensorShape& orig_left_shape,
               const TensorShape& orig_right_shape, bool transa = false,
               bool transb = false, bool trans_batch_a = false,
               bool trans_batch_b = false, bool fill_offsets = true) {
    ...
    // appending a 1 to its dimensions.After matrix multiplication the appended
    // 1 is removed.

    size_t left_num_dims = orig_left_shape.NumDimensions();
    size_t right_num_dims = orig_right_shape.NumDimensions();

    assert(left_num_dims >= 1 && right_num_dims >= 1);

    // Special cases below for right_shape being 2D and left_shape > 2D by
    // flattening left_shape to 2D Note that padding 1s in front of the
    // right_shape can be flattened too A: [M1, M2, ... K], B: [K, N] A: [M1,
    // M2, ... K], B: [N, K]^T A: [M1, M2, ... K], B: [1, ..., 1, K, N] A: [M1,
    // M2, ... K], B: [1, ..., 1, N, K]^T
    if (!transa && !trans_batch_a && !trans_batch_b && left_num_dims >= 2 &&
        right_num_dims >= 2 && left_num_dims >= right_num_dims &&
        orig_right_shape.SizeToDimension(right_num_dims - 1) ==
            orig_right_shape[right_num_dims - 2]) {
      M_ = static_cast<ptrdiff_t>(
          orig_left_shape.SizeToDimension(left_num_dims - 1));
      K_ = static_cast<ptrdiff_t>(orig_left_shape[left_num_dims - 1]);
      N_ =
          static_cast<ptrdiff_t>(transb ? orig_right_shape[right_num_dims - 2]
                                        : orig_right_shape[right_num_dims - 1]);
      output_shape_ = orig_left_shape;
      output_shape_[left_num_dims - 1] = N_;
      output_offsets_ = {0};
      left_offsets_ = {0};
      right_offsets_ = {0};
      assert((!transb && K_ == orig_right_shape[right_num_dims - 2]) ||
    ...
    if (trans_batch_a || trans_batch_b) {
      assert(left_num_dims > 2 && left_num_dims == right_num_dims);
      // trans_batch_a means the input tensor is either [M,b0,...,bn,K] or
      // [K,b0,...,bn,M]. Switch the dim[0] and batch dims here.
      if (trans_batch_a) {
      ...
      // trans_batch_b means the input tensor is either [K,b0,...,bn,N] or
      // [N,b0,...,bn,K]. Switch the dim[0] and batch dims here.
      if (trans_batch_b) {
    ...
    if (right_num_dims == 1) {
      ...
      right_padded_dims_[num_dims_with_pad - 2] =
          static_cast<ptrdiff_t>(right_shape[0]);

      if (num_input_dims >= 2) {
        // left padded to (...,1,K)
        TensorShapeCopyDims(left_shape, &left_padded_dims_[0],
                            left_num_dims - 2);
        left_padded_dims_[num_dims_with_pad - 3] = static_cast<ptrdiff_t>(
            left_shape[transa ? left_num_dims - 1 : left_num_dims - 2]);
        left_padded_dims_[num_dims_with_pad - 1] = static_cast<ptrdiff_t>(
            left_shape[transa ? left_num_dims - 2 : left_num_dims - 1]);
      } else {
        // pad 1 in the front
        TensorShapeCopyDims(
            left_shape, &left_padded_dims_[num_dims_with_pad - left_num_dims],
            left_num_dims);
      }
    } else {
      // pad 1 in the front for left
      TensorShapeCopyDims(left_shape,
                          &left_padded_dims_[num_dims_with_pad - left_num_dims],
                          left_num_dims);
      // pad 1 in the front for right
      TensorShapeCopyDims(
          right_shape, &right_padded_dims_[num_dims_with_pad - right_num_dims],
          right_num_dims);
    }
    ...
    for (size_t idx_dim = 0; idx_dim < num_dims_with_pad - 2; ++idx_dim) {
      output_dims[idx_dim] =
          std::max(left_padded_dims_[idx_dim], right_padded_dims_[idx_dim]);
      if (left_padded_dims_[idx_dim] != output_dims[idx_dim])
        assert(left_padded_dims_[idx_dim] == 1);
      if (right_padded_dims_[idx_dim] != output_dims[idx_dim])
        assert(right_padded_dims_[idx_dim] == 1);
    }
    if (transa) {
      M_ = static_cast<ptrdiff_t>(has_1D_input ? 1
                                               : left_shape[left_num_dims - 1]);
      K_ = static_cast<ptrdiff_t>(left_shape[left_num_dims - 2]);
    } else {
      M_ = static_cast<ptrdiff_t>(has_1D_input ? 1
                                               : left_shape[left_num_dims - 2]);
      K_ = static_cast<ptrdiff_t>(left_shape[left_num_dims - 1]);
    }

    if (transb) {
      N_ = static_cast<ptrdiff_t>(
          (right_num_dims == 1) ? 1 : right_shape[right_num_dims - 2]);
    } else {
      N_ = static_cast<ptrdiff_t>(
          (right_num_dims == 1) ? 1 : right_shape[right_num_dims - 1]);
    }

    if (!has_1D_input) {
      assert(K_ ==
             right_shape[transb ? right_num_dims - 1 : right_num_dims - 2]);
  ...
  bool Compute(const TensorShape& left_shape, const TensorShape& right_shape,
               const TensorShape* right_scale_shape,
               const TensorShape* right_zp_shape, bool transa = false,
               bool transb = false) {
    assert(Compute(left_shape, right_shape, transa, transb));
    ...
    auto SetRightMatrixQuantParam =
        [this, &right_shape](const TensorShape* param_shape,
                             std::vector<size_t>& quant_param_offsets) {
          if (nullptr != param_shape && param_shape->NumDimensions() > 1) {
            assert(param_shape->NumDimensions() ==
                       right_shape.NumDimensions() &&
                   param_shape->Size() * K_ == right_shape.Size());
            for (size_t batch_id = 0; batch_id < quant_param_offsets.size();
                 batch_id++) {
    ...
    assert(SetRightMatrixQuantParam(right_zp_shape, right_zp_offsets_));
    assert(SetRightMatrixQuantParam(right_scale_shape, right_scale_offsets_));

    return true;
  }

  // Move this piece of code to public so that we don't need to call this if we
  // can use GemmStridedBatched.
  void FillOffsets() {
    ...
    for (size_t i = num_broadcasted_dims_; i > 0; --i) {
      size_t idx = i - 1;
      output_broadcast_dims_[idx] =
          std::max(left_padded_dims_[idx], right_padded_dims_[idx]);
      output_broadcast_strides_[idx] =
          ((i == num_broadcasted_dims_) ? 1
                                        : output_broadcast_strides_[idx + 1] *
                                              output_broadcast_dims_[idx + 1]);
      left_padded_strides_[idx] =
          ((i == num_broadcasted_dims_)
               ? 1
               : left_padded_strides_[idx + 1] * left_padded_dims_[idx + 1]);
      right_padded_strides_[idx] =
          ((i == num_broadcasted_dims_)
               ? 1
               : right_padded_strides_[idx + 1] * right_padded_dims_[idx + 1]);
    }

    size_t num_offsets =
        output_broadcast_dims_[0] * output_broadcast_strides_[0];
 ...
 private:
  ...
  void RecursiveFill(size_t idx_dim, size_t idx_left, size_t idx_right,
                     size_t idx_out) {
    if (idx_dim == num_broadcasted_dims_) {
      ...
      for (int i = 0; i < output_dim; ++i) {
        RecursiveFill(
            ...
            idx_right +
                i * (right_dim == 1 ? 0 : right_padded_strides_[idx_dim]),
 ...
 public:
  // output shape
  const TensorShape& OutputShape() const { return output_shape_; }
  ...
  int Lda(bool is_trans) const {
    return (is_trans ? static_cast<int>(M_) : static_cast<int>(K_)) *
           left_ld_factor_;
  }

  int Ldb(bool is_trans) const {
    return (is_trans ? static_cast<int>(K_) : static_cast<int>(N_)) *
           right_ld_factor_;
  }

  int Ldc() const { return static_cast<int>(N_); }

  // Batched Gemm offsets in left matrices
  const std::vector<size_t>& LeftOffsets() const { return left_offsets_; }

  // Batched Gemm offsets in right matrices
  const std::vector<size_t>& RightOffsets() const { return right_offsets_; }

  // Batched Gemm offsets in output matrices
  const std::vector<size_t>& OutputOffsets() const { return output_offsets_; }
  ...
  template <typename T>
  static void OffsetToArrays(T* p, const std::vector<size_t>& offsets,
                             gsl::span<T*> arrays) {
    auto len = offsets.size();
    assert(arrays.size() == len);
  ...
  template <typename T>
  static void OffsetToArrays(const T* p, const std::vector<size_t>& offsets,
                             gsl::span<const T*> arrays) {
    auto len = offsets.size();
    assert(arrays.size() == len);
    for (size_t i = 0; i < len; i++) {
      arrays[i] = p + offsets[i];
...
#endif  // CUDNN_COMMON_MATMUL_HELPER_HPP

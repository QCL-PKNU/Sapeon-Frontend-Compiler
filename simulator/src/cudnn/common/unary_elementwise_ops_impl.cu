...
#define OP(name, expr)                                                    \
  template <typename T>                                                   \
  struct OP_##name {                                                      \
    __device__ __inline__ T operator()(const T& a) const { return expr; } \
  };

#define UNARY_ELEMENTWISE_IMPL(name)                                      \
  UNARY_ELEMENTWISE_IMPL_DECLARATION(name) {                              \
    UnaryElementWiseImpl(stream, input_data, output_data, OP_##name<T>(), \
                         count);                                          \
  }
...
SPECIALIZED_UNARY_ELEMENTWISE_IMPL_HFDB(Mish)
...
struct OP_Cast {
  __device__ __inline__ OutT operator()(const InT& a) const {
    const bool any_float16 =
        std::is_same<half, InT>::value || std::is_same<half, OutT>::value;
...
void Impl_Cast(cudaStream_t stream, const InT* input_data, OutT* output_data,
               size_t count) {
  UnaryElementWiseImpl(stream, input_data, output_data, OP_Cast<InT, OutT>(),
                       count);
}

#define SPECIALIZED_CAST_IMPL2(InT, OutT)                                      \
  template void Impl_Cast<InT, OutT>(cudaStream_t stream,                      \
                                     const InT* input_data, OutT* output_data, \
                                     size_t count);
